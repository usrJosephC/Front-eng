...
const SongSelection = () => {
  ...
  const [songs] = useState([
    { id: 1, title: "Halo", artist: "Beyoncé", year: 2008, selected: true },
    { id: 2, title: "TikTok", artist: "Kesha", year: 2009, selected: true },
    { id: 3, title: "Rolling In The Deep", artist: "Adele", year: 2010, selected: false },
    { id: 4, title: "Gangnam Style", artist: "Psy", year: 2012, selected: false },
  ]);

  const [selectedSongs, setSelectedSongs] = useState(
    songs.filter(song => song.selected).map(song => song.id)
  );
    ...
    <div className="min-h-screen bg-backgroundPurple text-white flex flex-col items-center justify-center px-4 text-center font-body">
      ...
      <button
        ...
        className={`mt-8 flex items-center gap-2 bg-spotifyYellow text-black font-semibold px-6 py-3 rounded-full ${
          selectedSongs.length === 0 ? 'opacity-50 cursor-not-allowed' : ''
        }`}
      >
